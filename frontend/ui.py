...
education = st.selectbox("Education/Educação", ['Bachelors','Some-college','11th',
...
occupation = st.selectbox("Occupation/Emprego",['Tech-support','Craft-repair','Other-service',
                        ...
                        'Armed-Forces'])
relationship = st.selectbox("Relationship/Relacionamento",['Wife','Own-child','Husband',
                            'Not-in-family','Other-relative','Unmarried'])
race = st.selectbox("Race/Raça",['White', 'Asian-Pac-Islander', 'Amer-Indian-Eskimo',
                                 'Other', 'Black'])
sex = st.selectbox("Sex/Sexo",['Female', 'Male'])
capital_gain = st.number_input("Capital Gain/Ganho de Capital", min_value=0,max_value=None)
capital_loss = st.number_input("Capital Loss/Perda de Capital", min_value=0,max_value=None)
hours_per_week = st.number_input("Hours per Week/Horas de trabalho", min_value=0, max_value=99)
native_country = st.selectbox("Native Country/País Nativo",['United-States', 'Cuba', 'Jamaica', 'India',
                            'Mexico','South','Puerto-Rico','Honduras','England','Canada','Germany',
                            'Iran','Philippines','Italy','Poland','Columbia','Cambodia','Thailand',
                            'Ecuador','Laos','Taiwan','Haiti','Portugal','Dominican-Republic','El-Salvador',
                            'France','Guatemala','China','Japan','Yugoslavia','Peru','Outlying-US(Guam-USVI-etc)',
                            'Scotland','Trinadad&Tobago','Greece','Nicaragua','Vietnam','Hong','Ireland',
...
if st.button("Predict"):
    response = requests.post('https://censussalary.herokuapp.com/predict/', json=data)
